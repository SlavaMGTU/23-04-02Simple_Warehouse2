...
class SW_Check(db.Entity):
    #good_id = Required("SW_Goods")
    barcode = Optional(str)
    price_tag = Optional(str)
    check = Optional(str)# 1 price Right; 0 prices are different


def init():
    db.generate_mapping(create_tables=True)
    with db_session:# add units '??' by default
        query = select(c for c in SW_Units if c.name=='??')[:]
        if query == []:
            i=SW_Units(name='??')
            commit()

    with db_session:# add SW_Check '?????????' by default
        query = select(c for c in SW_Check if c.barcode=='00608940553887')[:]
        if query == []:
            i=SW_Check(barcode='00608940553887', price_tag='120', check='1') # add DB pony SW_Check
            commit()