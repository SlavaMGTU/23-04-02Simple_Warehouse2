...
def right_price_cv(hashMap, _files=None, _data=None):#OnInput; Имя CV-шага: Просмотр цены
    ...
    if hashMap.get("listener") == "Следующий ценник":
        hashMap.put("NextStep", "osr_and_qr")

    return hashMap

def osr_and_qr_cv(hashMap, _files=None, _data=None):  # OnInput; Имя CV-шага: osr_and_qr
    if hashMap.get("listener") == 'ON_BACK_PRESSED':
        hashMap.put("NextStep", "Поиск товара")

    if hashMap.get("listener") == "Следующий ценник":
        hashMap.put("NextStep", "mat_object_osr")

    return hashMap

def mat_object_osr_cv(hashMap, _files=None, _data=None):  # OnInput; Имя CV-шага: mat_object_osr
    if hashMap.get("listener") == 'ON_BACK_PRESSED':
        hashMap.put("NextStep", "Поиск товара")

    if hashMap.get("listener") == "Следующий ценник":
        hashMap.put("NextStep", "mat_object_qr")

    return hashMap

def mat_object_qr_cv(hashMap, _files=None, _data=None):  # OnInput; Имя CV-шага: mat_object_qr_cv
    if hashMap.get("listener") == 'ON_BACK_PRESSED':
        hashMap.put("NextStep", "Поиск товара")

    if hashMap.get("listener") == "Следующий ценник":
        hashMap.put("NextStep", "multiscanner")

    return hashMap

def multiscanner_cv(hashMap, _files=None, _data=None):  # OnInput; Имя CV-шага: multiscanner_cv
    if hashMap.get("listener") == 'ON_BACK_PRESSED':
        hashMap.put("NextStep", "Поиск товара")
